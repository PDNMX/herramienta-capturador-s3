...
const {S3hcpf} = require("./modelHCPF");
...
module.exports = {
  create: async (req, res) => {
    try {
      ...
      let s3hcpf = new S3hcpf(newdocument);
      let result = await s3hcpf.save();
      ...
      res.status(200).json({
        message: "Se realizarón las inserciones correctamente en S3hcpf",
        objResponse,
      });
  ...
  getAll: async (req, res) => {
    try {
      ...
      const paginationResult = await S3hcpf.paginate(query, {
        page: page,
        limit: pageSize,
        sort: sortObj,
      }).then();
  ...
  list: async (req, res) => {
    try {
      ...
      res.status(200).json(objResponse);
      //res.status(200).json({ message: 'Se realizarón las inserciones correctamente en S3hcpf' });
    } catch (error) {
      return res.status(500).json({ message: "Error: ", error: error.message });
    }
  },
  update: async (req, res) => {
    try {
      ...
      let response;
      try {
        response = await S3hcpf.findByIdAndUpdate(id, newdocument, {
          upsert: false,
          new: true,
        }).exec();
      } catch (error) {
