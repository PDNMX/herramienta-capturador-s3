...
module.exports = {
  create: async (req, res) => {
    try {
      ...
      res
        .status(200)
        .json({
          message: "Se realizarón las inserciones correctamente en S3ang",
          objResponse,
        });
      /* res
        .status(200)
  ...
  getAll: async (req, res) => {
    try {
      ...
      const paginationResult = await S3ang.paginate(query, {
        page: page,
        limit: pageSize,
        sort: sortObj,
      }).then();
  ...
  update: async (req, res) => {
    try {
      ...
      const response = await S3ang.findByIdAndUpdate(id, newdocument, {
        upsert: false,
        new: true,
      }).exec();
