...
const {S3hcsp} = require("./modelHCSP");
...
module.exports = {
  create: async (req, res) => {
    try {
      ...
      let s3hcsp = new S3hcsp(newdocument);
      let result = await s3hcsp.save();
      ...
      res.status(200).json({
        message: "Se realizarón las inserciones correctamente en S3hcsp",
        objResponse,
      });
  ...
  getAll: async (req, res) => {
    try {
      ...
      const paginationResult = await S3hcsp.paginate(query, {
        page: page,
        limit: pageSize,
        sort: sortObj,
      }).then();
  ...
  list: async (req, res) => {
    try {
      ...
      res.status(200).json(objResponse);
      //res.status(200).json({ message: 'Se realizarón las inserciones correctamente en S3hcsp' });
    } catch (error) {
      return res.status(500).json({ message: "Error: ", error: error.message });
    }
  },
  update: async (req, res) => {
    try {
      ...
      let response;
      try {
        response = await S3hcsp.findByIdAndUpdate(id, newdocument, {
          upsert: false,
          new: true,
        }).exec();
      } catch (error) {
