...
const {S3hcpm} = require("./modelHCPM");
...
module.exports = {
  create: async (req, res) => {
    try {
      ...
      let s3hcpm = new S3hcpm(newdocument);
      let result = await s3hcpm.save();
      ...
      res.status(200).json({
        message: "Se realizarón las inserciones correctamente en SS3hcpm",
        objResponse,
      });
  ...
  getAll: async (req, res) => {
    try {
      ...
      const paginationResult = await S3hcpm.paginate(query, {
        page: page,
        limit: pageSize,
        sort: sortObj,
      }).then();
  ...
  list: async (req, res) => {
    try {
      ...
      res.status(200).json(objResponse);
      //res.status(200).json({ message: 'Se realizarón las inserciones correctamente en SS3hcpm' });
    } catch (error) {
      return res.status(500).json({ message: "Error: ", error: error.message });
    }
  },
  update: async (req, res) => {
    try {
      ...
      let response;
      try {
        response = await S3hcpm.findByIdAndUpdate(id, newdocument, {
          upsert: false,
          new: true,
        }).exec();
      } catch (error) {
