...
module.exports = {
  create: async (req, res) => {
    try {
      ...
      let s3pfg = new S3pfg(newdocument);
      let result = await s3pfg.save();
      ...
      res.status(200).json({
        message: "Se realizarón las inserciones correctamente en s3pfg",
        objResponse,
      });
  ...
  getAll: async (req, res) => {
    try {
      ...
      const paginationResult = await S3pfg.paginate(query, {
        page: page,
        limit: pageSize,
        sort: sortObj,
      }).then();
  ...
  list: async (req, res) => {
    try {
      ...
      res.status(200).json(objResponse);
      //res.status(200).json({ message: 'Se realizarón las inserciones correctamente en s3pfg' });
    } catch (error) {
      return res.status(500).json({ message: "Error: ", error: error.message });
    }
  },
  update: async (req, res) => {
    try {
      ...
      let response;
      try {
        response = await S3pfg.findByIdAndUpdate(id, newdocument, {
          upsert: false,
          new: true,
        }).exec();
      } catch (error) {
