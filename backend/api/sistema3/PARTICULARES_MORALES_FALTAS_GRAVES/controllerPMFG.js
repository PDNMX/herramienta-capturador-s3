...
const {S3pmfg} = require("./modelPMFG");
...
module.exports = {
  create: async (req, res) => {
    try {
       ...
       let s3pmfg = new S3pmfg(newdocument);
       let result = await s3pmfg.save();
       ...
       res.status(200).json({
         message: "Se realizarón las inserciones correctamente en S3pmfg",
         objResponse,
       });
  ...
  getAll: async (req, res) => {
    try {
      ...
      const paginationResult = await S3pmfg.paginate(query, {
        page: page,
        limit: pageSize,
        sort: sortObj,
      }).then();
  ...
  list: async (req, res) => {
    try {
      ...
      res.status(200).json(objResponse);
      //res.status(200).json({ message: 'Se realizarón las inserciones correctamente en S3pmfg' });
    } catch (error) {
      return res.status(500).json({ message: "Error: ", error: error.message });
    }
  },
  update: async (req, res) => {
    try {
      ...
      let response;
      try {
        response = await S3pmfg.findByIdAndUpdate(id, newdocument, {
          upsert: false,
          new: true,
        }).exec();
      } catch (error) {
