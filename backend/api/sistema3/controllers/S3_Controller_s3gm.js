const { S3gm } = require("../models/S3gm");
...
module.exports = {
  create_s3gm: async (req, res) => {
    try {
      ...
      let s3gm = new S3gm(newdocument);
      let result = await s3gm.save();
      ...
      res.status(200).json({ message: 'Se realizarón las inserciones correctamente en S3gm', objResponse });
  ...
  getAll_s3gm: async (req, res) => {
    try {
      ...
      const paginationResult = await S3gm.paginate(query, { page: page, limit: pageSize, sort: sortObj }).then();
  ...
  list_s3gm: async (req, res) => {
    try {
      ...
      res.status(200).json(objResponse); 
      //res.status(200).json({ message: 'Se realizarón las inserciones correctamente en S3gm' });
    } catch (error) {
      return res
        .status(500)
        .json({ message: "Error: ", error: error.message });
  ...
  update_s3gm: async (req, res) => {
    try {
      console.log("update_s3gm")
      ...
      let response;
      try{
        response = await S3gm.findByIdAndUpdate(id, newdocument, { upsert: false, new: true }).exec();
      }catch (error){
     ...
     /*  const values = req.body;
      ...
      let response = await S3gm.findOneAndUpdate(
        { _id: id },
        values,
        { new: true },
        (err, data) => {
