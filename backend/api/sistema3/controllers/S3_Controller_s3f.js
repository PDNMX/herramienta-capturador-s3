...
module.exports = {
  create_s3f: async (req, res) => {
    try {
      ...
      let s3f = new S3f(newdocument);
      let result = await s3f.save();
      ...
      res
        .status(200)
        .json({
          message: "Se realizarón las inserciones correctamente en s3f",
          objResponse,
        });
  ...
  getAll_s3f: async (req, res) => {
    try {
      ...
      const paginationResult = await S3f.paginate(query, {
        page: page,
        limit: pageSize,
        sort: sortObj,
      }).then();
  ...
  list_s3f: async (req, res) => {
    try {
      ...
      res.status(200).json(objResponse);
      //res.status(200).json({ message: 'Se realizarón las inserciones correctamente en s3f' });
    } catch (error) {
      return res.status(500).json({ message: "Error: ", error: error.message });
    }
  },
  update_s3f: async (req, res) => {
    try {
      ...
      let response;
      try {
          response = await S3f.findByIdAndUpdate(id, newdocument, {
          upsert: false,
          new: true,
        }).exec();
      } catch (error) {
      ...
      let response = await s3f.findOneAndUpdate(
        { _id: id },
        values,
        { new: true },
        (err, data) => {
