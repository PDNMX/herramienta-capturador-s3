...
module.exports = {
  create_s3ag: async (req, res) => {
    try {
      ...
      console.log('result._id:', result._id);
       const proveedorRegistros1 = new proveedorRegistros({ proveedorId: datausuario.proveedorDatos, registroSistemaId: result._id, sistema: 'S3ag', fechaCaptura:fecha, fechaActualizacion:fecha});      
  ...
  getAll_s3ag: async (req, res) => {
    try {
      ...
      const paginationResult = await S3ag.paginate(query, { page: page, limit: pageSize, sort: sortObj }).then();
  ...
  list_s3ag: async (req, res) => {
    try {
      ...
      res.status(200).json(objResponse); 
      //res.status(200).json({ message: 'Se realizarón las inserciones correctamente en s3ag' });
    } catch (error) {
      return res
        .status(500)
        .json({ message: "Error: ", error: error.message });
  ...
  update_s3ag: async (req, res) => {
    try {
      ...
      const response = await S3ag.findByIdAndUpdate(id, newdocument, { upsert: false, new: true }).exec();
     ...
     /*  const values = req.body;
      ...
      let response = await S3ag.findOneAndUpdate(
        { _id: id },
        values,
        { new: true },
        (err, data) => {
