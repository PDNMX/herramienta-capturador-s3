...
module.exports = {
  create_s3g: async (req, res) => {
    try {
      ...
      let s3g = new S3g(newdocument);
      let result = await s3g.save();
      ...
      //let proveedor = await proveedorRegistros.findOne({usuario: datausuario._id});
       ...
       res.status(200).json({ message: 'Se realizarón las inserciones correctamente en S3g', objResponse });
  ...
  getAll_s3g: async (req, res) => {
    try {
      ...
      const paginationResult = await S3g.paginate(query, { page: page, limit: pageSize, sort: sortObj }).then();
  ...
  list_s3g: async (req, res) => {
    try {
      ...
      res.status(200).json(objResponse); 
      //res.status(200).json({ message: 'Se realizarón las inserciones correctamente en S3g' });
    } catch (error) {
      return res
        .status(500)
        .json({ message: "Error: ", error: error.message });
  ...
  update_s3g: async (req, res) => {
    try {
      ...
      let response;
      try{
        response = await S3g.findByIdAndUpdate(id, newdocument, { upsert: false, new: true }).exec();
      }catch (error){
     ...
     /*  const values = req.body;
      ...
      let response = await S3g.findOneAndUpdate(
        { _id: id },
        values,
        { new: true },
        (err, data) => {
