...
module.exports = {
   ...
   */
      ...
      res
        .status(200)
        .json({
          message: "Se realizarón las inserciones correctamente en S3sfg",
          objResponse,
        });
      /* res
        .status(200)
  ...
  getAll: async (req, res) => {
    try {
      ...
      const paginationResult = await S3sfg.paginate(query, {
        page: page,
        limit: pageSize,
        sort: sortObj,
      }).then();
  ...
  list: async (req, res) => {
    try {
      ...
      res.status(200).json(objResponse);
      //res.status(200).json({ message: 'Se realizarón las inserciones correctamente en S3sfg' });
    } catch (error) {
      return res.status(500).json({ message: "Error: ", error: error.message });
    }
  },
  update: async (req, res) => {
    try {
      ...
      const response = await S3sfg.findByIdAndUpdate(id, newdocument, {
        upsert: false,
        new: true,
      }).exec();
