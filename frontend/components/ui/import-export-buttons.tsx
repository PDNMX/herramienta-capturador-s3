...
const SYSTEM_FIELDS = ["date_created", "date_updated", "user_created", "user_updated"];
...
export function ImportExportButtons({
  ...
  const handleExport = () => {
    ...
    toast({ title: "Exportación exitosa", description: `${data.length} registros exportados.` });
  };
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (evt) => {
      try {
        ...
        const clean = records.map(({ id, ...rest }) => {
          const obj: any = { ...rest };
          SYSTEM_FIELDS.forEach((f) => delete obj[f]);
          return obj;
        });
    ...
    <>
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end" className="w-44">
          <DropdownMenuItem onClick={handleExport} disabled={data.length === 0}>
