...
export const EmpleoCargoComisionGravesSection: React.FC<EmpleoCargoComisionGravesSectionProps> = ({
  form,
  loading,
}) => {
  ...
  return (
    <div className="space-y-6">
            ...
            <FormControl>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {[
                  ...
                  { value: "MUNICIPAL_ALCALDIA", label: "Municipal / Alcaldía" },
                ].map((option) => (
                  <div
                    key={option.value}
                  ...
                  >
                    <div className="flex items-start space-x-3 w-full">
                      <div className={`mt-0.5 h-4 w-4 rounded-full border-2 flex items-center justify-center ${nivelOrdenGobierno === option.value ? "border-primary" : "border-muted-foreground"}`}>
                        {nivelOrdenGobierno === option.value && <div className="h-2 w-2 rounded-full bg-primary" />}
                      </div>
                ...
                {[
                  ...
                  >
                    <div className="flex items-center space-x-2 w-full justify-center">
                      <div className={`h-4 w-4 rounded-full border-2 flex items-center justify-center flex-shrink-0 ${ambitoPublico === option.value ? "border-primary" : "border-muted-foreground"}`}>
                        {ambitoPublico === option.value && <div className="h-2 w-2 rounded-full bg-primary" />}
                      </div>
                      <Label className="font-medium cursor-pointer text-center">{option.label}</Label>
                    </div>
                  </div>
                ))}
              </div>
      ...
      {/* Valor del nivel jerarquico (cuando es OTRO) */}
