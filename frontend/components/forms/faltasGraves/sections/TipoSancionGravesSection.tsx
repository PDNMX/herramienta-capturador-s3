...
import { SuspensionEmpleoGravesFields } from "./tipoSancion/SuspensionEmpleoGravesFields";
import { DestitucionEmpleoGravesFields } from "./tipoSancion/DestitucionEmpleoGravesFields";
import { SancionEconomicaGravesFields } from "./tipoSancion/SancionEconomicaGravesFields";
import { InhabilitacionGravesFields } from "./tipoSancion/InhabilitacionGravesFields";
import { OtroSancionGravesFields } from "./tipoSancion/OtroSancionGravesFields";

interface TipoSancionGravesSectionProps {
  form: any;
  loading: boolean;
}
...
export const TipoSancionGravesSection: React.FC<TipoSancionGravesSectionProps> = ({
  form,
  loading,
}) => {
    ...
    <div className="space-y-6">
      ...
      {fields.map((field, index) => {
        ...
        return (
          <div
            key={field.id}
            ref={(el) => { if (el) containerRefs.current.set(field.id, el); else containerRefs.current.delete(field.id); }}
            className={`rounded-xl border-2 border-primary/20 p-6 bg-card/95 backdrop-blur shadow-lg relative border-l-4 border-l-primary/60 transition-all duration-300 hover:shadow-xl hover:border-primary/30 ${shouldAnimate ? "animate-array-item-enter" : ""}`}
          >
            {fields.length > 1 && (
              <Button
                type="button"
            ...
            <div className="space-y-6">
              ...
              {claveValue === "SUSPENSION" && (
                <SuspensionEmpleoGravesFields form={form} loading={loading} sancionIndex={index} />
              )}

              {claveValue === "DESTITUCION" && (
                <DestitucionEmpleoGravesFields form={form} loading={loading} sancionIndex={index} />
              )}

              {claveValue === "SANCION_ECONOMICA" && (
                <SancionEconomicaGravesFields form={form} loading={loading} sancionIndex={index} />
              )}

              {claveValue === "INHABILITACION" && (
                <InhabilitacionGravesFields form={form} loading={loading} sancionIndex={index} />
              )}

              {claveValue === "OTRO" && (
                <OtroSancionGravesFields form={form} loading={loading} sancionIndex={index} />
              )}
            </div>
          </div>
        );
