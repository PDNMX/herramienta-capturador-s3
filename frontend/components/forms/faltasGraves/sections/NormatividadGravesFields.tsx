...
interface NormatividadGravesFieldsProps {
  form: any;
  loading: boolean;
  faltaIndex: number;
}
...
export const NormatividadGravesFields: React.FC<NormatividadGravesFieldsProps> = ({
  form,
  loading,
  faltaIndex,
}) => {
      ...
      {fields.map((field, normIndex) => {
        ...
        return (
          <div
            ...
            className={`p-4 bg-muted/30 rounded-lg border border-muted relative border-l-[3px] border-l-primary/40 transition-all duration-300 ${shouldAnimate ? "animate-array-item-enter" : ""}`}
          >
            {fields.length > 1 && (
              <Button
                type="button"
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-2">
                ...
                <p className="text-sm font-semibold text-primary">
                  ...
                  {fields.length > 1 && <span className="text-xs font-normal text-muted-foreground ml-1.5">de {fields.length}</span>}
                </p>
              </div>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>Normatividad infringida <span className="text-red-500">*</span></FormLabel>
                    <FormControl>
                      <Combobox options={NORMATIVIDADES} value={field.value} onChange={field.onChange} placeholder="Selecciona la normatividad" disabled={loading} searchPlaceholder="Buscar normatividad..." />
                    </FormControl>
                    <FormDescription>Escribir el nombre de la normatividad infringida</FormDescription>
                    <FormMessage />
                  </FormItem>
                )}
              />
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>Artículo(s) <span className="text-red-500">*</span></FormLabel>
                      <FormControl>
                        <Input disabled={loading} placeholder="Ej: 34, 35, 36" {...field} value={field.value || ""} />
                      </FormControl>
                      <FormDescription>Escribir el (los) artículo(s) infringido(s)</FormDescription>
                      <FormMessage />
                    </FormItem>
                  )}
                />
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Input disabled={loading} placeholder="Ej: I, II, III (si aplica)" {...field} value={field.value || ""} />
                      </FormControl>
                      <FormDescription>En su caso, escribir la(s) fracción(es) infringida(s)</FormDescription>
                      <FormMessage />
                    </FormItem>
                  )}
                />
