...
interface OrigenProcedimientoGravesSectionProps {
  form: any;
  loading: boolean;
}

export const OrigenProcedimientoGravesSection: React.FC<
  OrigenProcedimientoGravesSectionProps
> = ({ form, loading }) => {
  ...
  return (
    <div className="space-y-6">
      <p className="text-sm text-muted-foreground">
        ...
        investigacion por actos vinculados con faltas administrativas graves
        por parte del servidor publico
      </p>
      ...
      <FormField
        ...
        render={({ field }) => (
          <FormItem className="space-y-3">
            ...
            <FormDescription>
              Seleccionar conforme al catalogo el origen de la falta administrativa
            </FormDescription>
            <FormControl>
              <div className="space-y-3">
                {opciones.map((opcion) => (
                  <div
                    key={opcion.value}
                    onClick={() => !loading && handleClaveChange(opcion.value)}
                    className={`
                      relative flex cursor-pointer rounded-xl border-2 p-4 hover:bg-accent transition-colors
                      ${claveOrigen === opcion.value ? "border-primary bg-accent" : "border-muted"}
                  ...
                  >
                    <div className="flex items-start space-x-3 w-full">
                      <div className={`
                        ...
                        ${claveOrigen === opcion.value ? "border-primary" : "border-muted-foreground"}
                      `}>
                        {claveOrigen === opcion.value && (
