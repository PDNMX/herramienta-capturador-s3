...
END $$;
-- FLOWS PARA VALIDACIÓN DE STATUS FIRME CON RESOLUCIONES COMPLETAS

-- FLOW PARA faltas_administrativas_graves
\echo '🔄 Configurando flow de validación de status FIRME para faltas_administrativas_graves...'
DO $$
DECLARE
    admin_id uuid;
    validation_code_admin_graves_status text;
    flow_id_admin_graves_status uuid := 'ffffffff-ffff-ffff-ffff-ffffffffffff';
    operation_id_admin_graves_status uuid := 'f0000000-0000-0000-0000-000000000001';
BEGIN
    -- Obtener ID del administrador
    SELECT u.id INTO admin_id 
    FROM directus_users u
    ...
    -- Código de validación para faltas_administrativas_graves
    validation_code_admin_graves_status := 'module.exports=async function(data,{services,getSchema}){const{ItemsService}=services;const payload=data.$trigger?.payload||data;const status=payload.status;const resolucionId=payload.resolucion;if(status!=="FIRME")return data;if(!resolucionId){throw new Error("Debe asociar una resolución antes de cambiar el status a FIRME");}const schema=await getSchema();const resolucionService=new ItemsService("resolucion",{schema,accountability:data.$accountability});let resolucion;try{resolucion=await resolucionService.readOne(resolucionId);}catch(e){throw new Error("No se pudo obtener la información de la resolución");}const camposObligatorios={"Título de Resolución":resolucion.tituloResolucion,"Fecha de Resolución":resolucion.fechaResolucion,"Fecha de Notificación":resolucion.fechaNotificacion,"URL de Resolución":resolucion.urlResolucion,"Fecha de Resolución Firme":resolucion.fechaResolucionFirme,"Fecha de Notificación Firme":resolucion.fechaNotificacionFirme,"URL de Resolución Firme":resolucion.urlResolucionFirme,"Orden Jurisdiccional":resolucion.ordenJurisdiccional,"Autoridad Resolutora":resolucion.autoridadResolutora,"Autoridad Investigadora":resolucion.autoridadInvestigadora,"Autoridad Substanciadora":resolucion.autoridadSubstanciadora};const camposFaltantes=Object.entries(camposObligatorios).filter(([nombre,valor])=>!valor||valor==="").map(([nombre])=>nombre);if(camposFaltantes.length>0){throw new Error(`No se puede cambiar el status a FIRME. Faltan los siguientes campos en la resolución: ${camposFaltantes.join(", ")}`);}return data;};';
    ...
    INSERT INTO directus_flows (
        ...
        flow_id_admin_graves_status,
        'validar-status-firme-admin-graves',
        'check_circle',
        '#4CAF50',
        'Validar que la resolución esté completa antes de cambiar status a FIRME en faltas_administrativas_graves',
        'active',
        'event',
        NULL,
        '{"type":"filter","scope":["items.create","items.update"],"collections":["faltas_administrativas_graves"]}',
        operation_id_admin_graves_status,
        CURRENT_TIMESTAMP,
        admin_id
    ) ON CONFLICT (id) DO UPDATE SET
        options = EXCLUDED.options,
    ...
    INSERT INTO directus_operations (
        ...
        operation_id_admin_graves_status,
        'validar-status-firme-admin-graves-op',
        'validar_status_firme_admin_graves',
        'exec',
        19,
        1,
        json_build_object('code', validation_code_admin_graves_status),
        NULL,
        NULL,
        flow_id_admin_graves_status,
        CURRENT_TIMESTAMP,
    ...
    RAISE NOTICE '✅ Flow de validación de status FIRME para faltas_administrativas_graves configurado';
END $$;

-- FLOW PARA faltas_administrativas_no_graves
\echo '🔄 Configurando flow de validación de status FIRME para faltas_administrativas_no_graves...'
DO $$
DECLARE
    admin_id uuid;
    validation_code_admin_no_graves_status text;
    flow_id_admin_no_graves_status uuid := 'f0000000-0000-0000-0000-000000000002';
    operation_id_admin_no_graves_status uuid := 'f0000000-0000-0000-0000-000000000003';
BEGIN
    -- Obtener ID del administrador
    SELECT u.id INTO admin_id 
    FROM directus_users u
    ...
    -- Código de validación para faltas_administrativas_no_graves
    validation_code_admin_no_graves_status := 'module.exports=async function(data,{services,getSchema}){const{ItemsService}=services;const payload=data.$trigger?.payload||data;const status=payload.status;const resolucionId=payload.resolucion;if(status!=="FIRME")return data;if(!resolucionId){throw new Error("Debe asociar una resolución antes de cambiar el status a FIRME");}const schema=await getSchema();const resolucionService=new ItemsService("resolucion_no_graves",{schema,accountability:data.$accountability});let resolucion;try{resolucion=await resolucionService.readOne(resolucionId);}catch(e){throw new Error("No se pudo obtener la información de la resolución");}const camposObligatorios={"Título de Resolución":resolucion.tituloResolucion,"Fecha de Resolución":resolucion.fechaResolucion,"Fecha de Notificación":resolucion.fechaNotificacion,"Fecha de Resolución Firme":resolucion.fechaResolucionFirme,"Fecha de Notificación Firme":resolucion.fechaNotificacionFirme,"Autoridad Resolutora":resolucion.autoridadResolutora,"Autoridad Investigadora":resolucion.autoridadInvestigadora,"Autoridad Substanciadora":resolucion.autoridadSubstanciadora};const camposFaltantes=Object.entries(camposObligatorios).filter(([nombre,valor])=>!valor||valor==="").map(([nombre])=>nombre);if(camposFaltantes.length>0){throw new Error(`No se puede cambiar el status a FIRME. Faltan los siguientes campos en la resolución: ${camposFaltantes.join(", ")}`);}return data;};';
    ...
    INSERT INTO directus_flows (
        ...
        flow_id_admin_no_graves_status,
        'validar-status-firme-admin-no-graves',
        'check_circle',
        '#8BC34A',
        'Validar que la resolución esté completa antes de cambiar status a FIRME en faltas_administrativas_no_graves',
        'active',
        'event',
        NULL,
        '{"type":"filter","scope":["items.create","items.update"],"collections":["faltas_administrativas_no_graves"]}',
        operation_id_admin_no_graves_status,
        CURRENT_TIMESTAMP,
        admin_id
    ) ON CONFLICT (id) DO UPDATE SET
        options = EXCLUDED.options,
    ...
    INSERT INTO directus_operations (
        ...
        operation_id_admin_no_graves_status,
        'validar-status-firme-admin-no-graves-op',
        'validar_status_firme_admin_no_graves',
        'exec',
        19,
        1,
        json_build_object('code', validation_code_admin_no_graves_status),
        NULL,
        NULL,
        flow_id_admin_no_graves_status,
        CURRENT_TIMESTAMP,
    ...
    RAISE NOTICE '✅ Flow de validación de status FIRME para faltas_administrativas_no_graves configurado';
END $$;

-- FLOW PARA faltas_graves_personas_morales
\echo '🔄 Configurando flow de validación de status FIRME para faltas_graves_personas_morales...'
DO $$
DECLARE
    admin_id uuid;
    validation_code_morales_status text;
    flow_id_morales_status uuid := 'f0000000-0000-0000-0000-000000000004';
    operation_id_morales_status uuid := 'f0000000-0000-0000-0000-000000000005';
BEGIN
    -- Obtener ID del administrador
    SELECT u.id INTO admin_id 
    FROM directus_users u
    ...
    -- Código de validación para faltas_graves_personas_morales
    validation_code_morales_status := 'module.exports=async function(data,{services,getSchema}){const{ItemsService}=services;const payload=data.$trigger?.payload||data;const status=payload.status;const resolucionId=payload.resolucion;if(status!=="FIRME")return data;if(!resolucionId){throw new Error("Debe asociar una resolución antes de cambiar el status a FIRME");}const schema=await getSchema();const resolucionService=new ItemsService("resolucion_morales",{schema,accountability:data.$accountability});let resolucion;try{resolucion=await resolucionService.readOne(resolucionId);}catch(e){throw new Error("No se pudo obtener la información de la resolución");}const camposObligatorios={"Título de Resolución":resolucion.tituloResolucion,"Fecha de Resolución":resolucion.fechaResolucion,"Fecha de Notificación":resolucion.fechaNotificacion,"URL de Resolución":resolucion.urlResolucion,"Fecha de Resolución Firme":resolucion.fechaResolucionFirme,"Fecha de Notificación Firme":resolucion.fechaNotificacionFirme,"URL de Resolución Firme":resolucion.urlResolucionFirme,"Orden Jurisdiccional":resolucion.ordenJurisdiccional,"Autoridad Resolutora":resolucion.autoridadResolutora,"Autoridad Investigadora":resolucion.autoridadInvestigadora,"Autoridad Substanciadora":resolucion.autoridadSubstanciadora};const camposFaltantes=Object.entries(camposObligatorios).filter(([nombre,valor])=>!valor||valor==="").map(([nombre])=>nombre);if(camposFaltantes.length>0){throw new Error(`No se puede cambiar el status a FIRME. Faltan los siguientes campos en la resolución: ${camposFaltantes.join(", ")}`);}return data;};';
    ...
    INSERT INTO directus_flows (
        ...
        flow_id_morales_status,
        'validar-status-firme-morales',
        'check_circle',
        '#FF9800',
        'Validar que la resolución esté completa antes de cambiar status a FIRME en faltas_graves_personas_morales',
        'active',
        'event',
        NULL,
        '{"type":"filter","scope":["items.create","items.update"],"collections":["faltas_graves_personas_morales"]}',
        operation_id_morales_status,
        CURRENT_TIMESTAMP,
        admin_id
    ) ON CONFLICT (id) DO UPDATE SET
        options = EXCLUDED.options,
    ...
    INSERT INTO directus_operations (
        ...
        operation_id_morales_status,
        'validar-status-firme-morales-op',
        'validar_status_firme_morales',
        'exec',
        19,
        1,
        json_build_object('code', validation_code_morales_status),
        NULL,
        NULL,
        flow_id_morales_status,
        CURRENT_TIMESTAMP,
    ...
    RAISE NOTICE '✅ Flow de validación de status FIRME para faltas_graves_personas_morales configurado';
END $$;

-- FLOW PARA faltas_graves_personas_fisicas
\echo '🔄 Configurando flow de validación de status FIRME para faltas_graves_personas_fisicas...'
DO $$
DECLARE
    admin_id uuid;
    validation_code_fisicas_status text;
    flow_id_fisicas_status uuid := 'f0000000-0000-0000-0000-000000000006';
    operation_id_fisicas_status uuid := 'f0000000-0000-0000-0000-000000000007';
BEGIN
    -- Obtener ID del administrador
    SELECT u.id INTO admin_id 
    FROM directus_users u
    ...
    -- Código de validación para faltas_graves_personas_fisicas
    validation_code_fisicas_status := 'module.exports=async function(data,{services,getSchema}){const{ItemsService}=services;const payload=data.$trigger?.payload||data;const status=payload.status;const resolucionId=payload.resolucion;if(status!=="FIRME")return data;if(!resolucionId){throw new Error("Debe asociar una resolución antes de cambiar el status a FIRME");}const schema=await getSchema();const resolucionService=new ItemsService("resolucion_fisica",{schema,accountability:data.$accountability});let resolucion;try{resolucion=await resolucionService.readOne(resolucionId);}catch(e){throw new Error("No se pudo obtener la información de la resolución");}const camposObligatorios={"Título de Resolución":resolucion.tituloResolucion,"Fecha de Resolución":resolucion.fechaResolucion,"Fecha de Notificación":resolucion.fechaNotificacion,"URL de Resolución":resolucion.urlResolucion,"Fecha de Resolución Firme":resolucion.fechaResolucionFirme,"Fecha de Notificación Firme":resolucion.fechaNotificacionFirme,"URL de Resolución Firme":resolucion.urlResolucionFirme,"Orden Jurisdiccional":resolucion.ordenJurisdiccional,"Autoridad Resolutora":resolucion.autoridadResolutora,"Autoridad Investigadora":resolucion.autoridadInvestigadora,"Autoridad Substanciadora":resolucion.autoridadSubstanciadora};const camposFaltantes=Object.entries(camposObligatorios).filter(([nombre,valor])=>!valor||valor==="").map(([nombre])=>nombre);if(camposFaltantes.length>0){throw new Error(`No se puede cambiar el status a FIRME. Faltan los siguientes campos en la resolución: ${camposFaltantes.join(", ")}`);}return data;};';
    ...
    INSERT INTO directus_flows (
        ...
        flow_id_fisicas_status,
        'validar-status-firme-fisicas',
        'check_circle',
        '#E91E63',
        'Validar que la resolución esté completa antes de cambiar status a FIRME en faltas_graves_personas_fisicas',
        'active',
        'event',
        NULL,
        '{"type":"filter","scope":["items.create","items.update"],"collections":["faltas_graves_personas_fisicas"]}',
        operation_id_fisicas_status,
        CURRENT_TIMESTAMP,
        admin_id
    ) ON CONFLICT (id) DO UPDATE SET
        options = EXCLUDED.options,
    ...
    INSERT INTO directus_operations (
        ...
        operation_id_fisicas_status,
        'validar-status-firme-fisicas-op',
        'validar_status_firme_fisicas',
        'exec',
        19,
        1,
        json_build_object('code', validation_code_fisicas_status),
        NULL,
        NULL,
        flow_id_fisicas_status,
        CURRENT_TIMESTAMP,
    ...
    RAISE NOTICE '✅ Flow de validación de status FIRME para faltas_graves_personas_fisicas configurado';
END $$;
EOF
